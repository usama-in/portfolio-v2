...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden">
          ...
          <div className="flex justify-center space-x-6 mb-16 animate-slide-up stagger-5">
            {[
              ...
              <a
                key={social.label}
                href={social.href}
                className={`p-3 bg-slate-800/50 rounded-full text-slate-400 hover:text-white hover:bg-blue-500/20 transition-all duration-300 hover-lift animate-scale-in stagger-${index + 1}`}
      ...
      <div className="absolute bottom-40 left-20 w-12 h-12 bg-cyan-500/10 rounded-full animate-float" style={{ animationDelay: '4s' }} />
      <div className="absolute bottom-20 right-10 w-24 h-24 bg-pink-500/10 rounded-full animate-float" style={{ animationDelay: '1s' }} />
    </section>
  );
};
